...
export class MatchplayComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    of(this.matchPlayData)
      ...
      .subscribe(
        async (data) => {
          let dataLeaderboard = await this.facadeService.MatchPlayDataQuery(
            "-L6WPki8tSDZ1IAAoRXZ",
            this.tournamentID
          );
          ...
          if (this.matchPlayData["matchFormat"] == matchFormat.TEXAS_SCRAMBLE) {
            this.showTaxes = true;
          }
          ...
          for (let round = 1; round <= tournamentData.noOfRounds; round++) {
            let r: any = {
              Text: "Round " + round,
              Value: round,
            };
    ...
    if (tournamentData.noOfRounds > 0) {
      if (this.ddSelectedFlight != "0") {
        this.roundFlights = tournamentData.FlightsQL.filter((a) => {
          return (
            a.flightRound == this.flightRound && a.id == this.ddSelectedFlight
          );
        });
      } else {
        this.roundFlights = tournamentData.FlightsQL.filter((a) => {
      ...
      if (!this.showTaxes) {
        ...
        if (!this.selectedTeamName) {
          ...
          var filteredArray: any = this.roundFlights.filter((element) => {
            if (
      ...
      for (let hole of courseHoleQLs) {
        ...
        if (holeObj) {
          ...
          if (grossScore) {
            ...
            let subTournamentMember = allSubTournamentMember.filter(
              (x) => x.playerId == player.playerId
            );

            if (subTournamentMember.length > 0) {
              for (let subScore of subTournamentMember) {
                let subFlightScore = Object.assign({}, playerScore);
                subFlightScore.flightId = subScore.flightId;
                console.log(
                  subFlightScore.flightId + " ### " + playerScore.flightId
                );
            ...
            if (grossScore > 9) player2DigitIds.push(hole.id + "&" + player.playerId);
        ...
        for (let hole of courseHoleQLs) {
          ...
          if (holeObj) {
            ...
            if (grossScore) {
              ...
              if (grossScore > 9) player2DigitIds.push(hole.id + "&" + flightId);
              else player1DigitIds.push(hole.id + "&" + flightId);
  
              totalPlayed++;
            } else playerEmptyScoresIds.push(hole.id + "&" + flightId);
          }
        }
        console.log(playerScores);
        console.log(playerScoresIds);
    ...
    if (this.subTournaments.length > 0) {
      ...
      for (let subTournamentsQL of this.subTournaments) {
        ...
        for (let subTournamentFlightsQL of subTournamentFlightsQLs) {
          if (currentflightRound == subTournamentFlightsQL.flightRound) {
