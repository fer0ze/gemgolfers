...
export class ViewDailyRoundComponent implements OnInit {
      ...
      if (
        ...
        if (flight.noOfRounds > 0) {
          ...
          if (this.filterPlayer) {
            var filteredArray: any = flight.FlightsQL.filter((element) =>
              element.MembersQL.some(
                (MembersQL) =>
                  this.filterPlayer
                    .toLowerCase()
                    .includes(
                      MembersQL.PlayerQL.firstName.toLowerCase().trim()
                    ) ||
                  MembersQL.PlayerQL.membershipNumber == this.filterPlayer
              )
            ).map((element) => {
                ...
                MembersQL: element.MembersQL.filter(
                  (subElement) =>
                    this.filterPlayer
                      .toLowerCase()
                      .includes(
                        subElement.PlayerQL.firstName.toLowerCase().trim()
                      ) ||
                    subElement.PlayerQL.membershipNumber == this.filterPlayer
                ),
              });
              return n;
            });
  ...
  private async setupMatchplay(
    ...
    for (let flightData of flightsQLs) {
      ...
      this.flightPlayers[this.flightPlayers.length - 1]["flightTime"] =
        flightData.time;
      this.flightPlayers[this.flightPlayers.length - 1]["courseHoleSetTitle"] =
        courseHoleSetTitle ? courseHoleSetTitle.displayName : "";
      this.flightPlayers[this.flightPlayers.length - 1]["courseHoleSetKey"] =
        courseHoleSetTitle
    ...
    for (let i = 0; i < holes.length; i++) {
      ...
      if (holeNo >= 1 && holeNo <= 9) {
        ...
        if (!holes28to36) {
          ...
          let setsToRemove: number =
            (holes1to9 ? 0 : 1) + (holes10to18 ? 0 : 1) + (holes19to27 ? 0 : 1);
          if (setsToRemove > 0) {
            //holes.get(i).setHoleNo(holeNo - 9 * setsToRemove);
            holes[i].holeNo = holeNo - 9 * setsToRemove;
          }
  ...
  private async setupMatchplayData(
    ...
    for (let flightData of flightsQLs) {
      ...
      for (let membersQL of membersQLs) {
        ...
        this.duplicateIds.push(membersQL.playerId + "<>" + membersQL.flightId);
        let playerId: String = player.id;
      ...
      this.flightPlayers[this.findex]["courseHoleSetTitle"] = courseHoleSetTitle
        ? courseHoleSetTitle.displayName
        : "";
      this.flightPlayers[this.findex]["courseHoleSetKey"] = courseHoleSetTitle
        ? flightData.courseHoleSets + "_" + flightData.courseHoleSetsInverted
        : "";
        ...
        if (holeObj) {
          ...
          if (grossScore) {
            let playerScore: Score = {
              ...
              holeId: hole.id,
              playerHandicap: this.precisionRound(player.handicap, 0),
            ...
            if (grossScore > 9)
              player2DigitIds.push(hole.id + "&" + player.playerId);
            else player1DigitIds.push(hole.id + "&" + player.playerId);

            totalPlayed++;
          } else playerEmptyScoresIds.push(hole.id + "&" + player.playerId);
        }
      }
  ...
  onGross9Change(grossValue: string, playerId: string, header: any): void {
    ...
    for (let hole of header.courseHoles9) {
      if (<HTMLInputElement>document.getElementById(hole.id + "&" + playerId))
        console.log(
          (<HTMLInputElement>document.getElementById(hole.id + "&" + playerId))
            .value
        );
      total9 +=
        (<HTMLInputElement>document.getElementById(hole.id + "&" + playerId))
  ...
  async changeCourseHoleset(flightId: string, tournamentID: string) {
    ...
    if (!updatedData) return;
      ...
      if (updatedData.FlightsQL.length > 0) flight = updatedData.FlightsQL[0];
    }
    ...
    //             ? result.members[members].playingTee
    //             : result.members[members].playerCategory.toUpperCase()
    //         );
    ...
    //           let dataLeaderboard =
    //             await this.facadeService.getCourseInformation(flight.courseId);
    ...
    //         // }
    //       } else {
    //         let result = await this.facadeService.updateDailyRoundCourseHoleset(
    //           flight.tournamentId,
    //           holeSetsSelection,
    //           holeSetsInverted,
    //           false,
    ...
    //     let updatedFlight = await this.facadeService.singleRoundFlightsQuery(
    //       flightId
    //     );
    ...
    //     ) {
    //       courseHoleSetTitle =
    //         updatedFlight.FlightsQL[0].CourseQL.CourseHoleSetsQL.find((a) => {
    //           return (
    //             a.holeSets == updatedFlight.FlightsQL[0].courseHoleSets &&
    //             a.inverted == updatedFlight.FlightsQL[0].courseHoleSetsInverted
    //           );
    //         });
    //     }
    //     console.log(courseHoleSetTitle);
    ...
    //       courseHoleSetTitle.displayName;
    //     this.flightPlayers[flightIndex]["courseHoleSetKey"] = updatedFlight
    //       .FlightsQL[0]
    //       ? updatedFlight.FlightsQL[0].courseHoleSets +
    //         "_" +
    //         updatedFlight.FlightsQL[0].courseHoleSetsInverted
  ...
  async saveFlightChanges(flightId) {
    ...
    if (currentFlight) {
      ...
      this.flightPlayers[flightIndex] = singleFlight;
      this.flightPlayers[flightIndex]["header"] = flightHeader;
      this.flightPlayers[flightIndex]["flightId"] = updatedData.FlightsQL[0].id;
      this.flightPlayers[flightIndex]["tournamentId"] = updatedData.id;
      this.flightPlayers[flightIndex]["flightTime"] = time;
      this.flightPlayers[flightIndex]["courseHoleSetTitle"] =
        courseHoleSetTitle.displayName;
      this.flightPlayers[flightIndex]["courseHoleSetKey"] = courseHoleSetTitle
        ? courseHoleSetTitle.holeSets + "_" + courseHoleSetTitle.inverted
        : "";
      this.flightPlayers[flightIndex]["courseTee"] = courseHoleSetTitle
        ? updatedFlight.FlightsQL[0].tee
        : "";
  ...
  createNewRound(player) {
    ...
    //       courseId: player.courseId,
    //       adminId: this.loggedInuser.id,
    //       title: this.fDate + " " + this.loggedInuser.membership[0].club.name,
    //       prefix: null,
    //       courseHoleSets: addRound.holeSets,
    //       teamMatch: false,
    //       pairsMatch: false,
    ...
    //       activeRound: 1,
    //       playingOnWhs:
    //         this.dailyData && this.dailyData.TournamentsQL.length > 0
    ...
    //       tee: selectedFlight.courseTee ? selectedFlight.courseTee : "AMATEURS",
    //       tee_id: 1,
    ...
    //         let playerIndex = this.flightPlayers[flightIndex].findIndex(
    ...
    //         let updatedFlight =
    //           await this.facadeService.singleRoundFlightsQuery(newFlightId);
    //         console.log(updatedFlight);
    ...
    //           courseHoleSetTitle.displayName;
    //         newFlightData["courseHoleSetKey"] = updatedFlight.FlightsQL[0]
    //           ? updatedFlight.FlightsQL[0].courseHoleSets +
    //             "_" +
    //             updatedFlight.FlightsQL[0].courseHoleSetsInverted
    //           : "";
    ...
    //         this.flightPlayers.splice(flightIndex, 0, newFlightData);

    //         this.snackBar.open("Player has been moved to a new round.", "x", {
    //           duration: 5000,
    //         });
    //       }
