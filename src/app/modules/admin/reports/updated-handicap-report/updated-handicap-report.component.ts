  ...
  getPlayerUpdatedHandicapReportCongu() {
    ...
    if (this.dataPlayersCongu.player_handicap) {
      for (
        ...
        const dailyStat = {
          name:
            this.dataPlayersCongu.player_handicap[this.index].PlayerQL
              .firstName +
            " " +
            this.dataPlayersCongu.player_handicap[this.index].PlayerQL.lastName,
          membership:
            this.dataPlayersCongu.player_handicap[this.index].PlayerQL
              .membershipNumber,
          date: this.dataPlayersCongu.player_handicap[this.index].tournamentQL
            ? this.dataPlayersCongu.player_handicap[this.index].tournamentQL
                .endDate
            : "",
          handicap: this.dataPlayersCongu.player_handicap[this.index].handicap,
          oldHandicap:
            this.dataPlayersCongu.player_handicap[this.index].oldHandicap,
          grossScore:
            this.dataPlayersCongu.player_handicap[this.index].grossScore,
          adjustedScore:
            this.dataPlayersCongu.player_handicap[this.index].adjustedScore,
          score: this.dataPlayersCongu.player_handicap[this.index].score,
        };
  ...
  getPlayerUpdatedHandicapReportWHS() {
    ...
    if (this.dataPlayersWHS.player_handicap_whs) {
      for (
        ...
        const dailyStat = {
          name:
            this.dataPlayersWHS.player_handicap_whs[this.index].player
              .firstName +
            " " +
            this.dataPlayersWHS.player_handicap_whs[this.index].player.lastName,
          membership:
            this.dataPlayersWHS.player_handicap_whs[this.index].player
              .membershipNumber,
          date: this.dataPlayersWHS.player_handicap_whs[this.index].tournament
            ? this.dataPlayersWHS.player_handicap_whs[this.index].tournament
                .endDate
            : "",
          handicapIndex:
            this.dataPlayersWHS.player_handicap_whs[this.index].handicapIndex,
          score: this.dataPlayersWHS.player_handicap_whs[this.index].score,
          handicapDifferential:
            this.dataPlayersWHS.player_handicap_whs[this.index]
              .handicapDifferential,
          adjustedScore:
            this.dataPlayersWHS.player_handicap_whs[this.index].adjustedScore,
        };
      ...
      if (filterValue.length >= 3) {
        for (let c of this.dataPlayersCongu.player_handicap) {
          if (c.PlayerQL.membershipNumber == filterValue) {
            const dailyStat = {
              name: c.PlayerQL.firstName + " " + c.PlayerQL.lastName,
      ...
      if (filterValue.length >= 3) {
        for (let c of this.dataPlayersCongu.player_handicap) {
          ...
          if (
            ...
            c["fullname"].toLowerCase().toString().includes(filterValue) ||
            (c.PlayerQL.email &&
              c.PlayerQL.email
                .toLowerCase()
                .toString()
                .includes(filterValue)) ||
            (c.PlayerQL.phone &&
              c.PlayerQL.phone.toLowerCase().toString().includes(filterValue))
          ) {
            const dailyStat = {
              name: c.PlayerQL.firstName + " " + c.PlayerQL.lastName,
              membership: c.PlayerQL.membershipNumber,
      ...
      if (filterValue.length >= 3) {
        for (let c of this.dataPlayersWHS.player_handicap_whs) {
          ...
          if (
            ...
            (c.player.phone &&
              c.player.phone.toLowerCase().toString().includes(filterValue))
          ) {
            const dailyStat = {
              name: c.player.firstName + " " + c.player.lastName,
              membership: c.player.membershipNumber,
