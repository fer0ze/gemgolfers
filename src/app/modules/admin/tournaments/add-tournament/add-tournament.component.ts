...
export class AddTournamentComponent implements OnInit {
    ...
    */
    ...
    if (this.tournamentID) {
      ...
      if (this.currentTournament) {
        ...
        this.formArray
          ...
          .get([0])
          .get("courseName")
          .setValue({ name: this.currentTournament["CourseQL"].name });
        ...
        for (let p of playerCategoryList) {
          let founded = this.currentTournament["CategoriesQL"].filter((a) => {
    ...
    this.filteredCourseOptions = this.formArray
      ...
      .valueChanges.pipe(
        ...
        map((name) => (name ? this._filterCourse(name) : this.Courses.slice()))
      );
    ...
    if (this.loggedInuser.userRole > 1) {
      ...
      this.formArray.get([0]).get("clubsFormCtrl").updateValueAndValidity();
    }
  }
  ...
  createCategory(cat: any): FormGroup {
    return this._formBuilder.group({
      ...
      selectedcategories: [this.formArray.get([0]).get("clubctgies").value],
  ...
  updateChkbxArray(chk, index, isChecked, key) {
    ...
    if (isChecked) {
      ...
      let idx = chkArray.controls.findIndex((x) => x.value.name == chk.name);
  ...
  updateCourseHolSet(chk, isChecked, key) {
    ...
    if (isChecked) {
      ...
      if (chkArray.controls.findIndex((x) => x.value.id == chk.id) == -1)
        chkArray.push(
          new FormControl({ id: chk.id, name: chk.name, checked: true })
        );
    } else {
  ...
  onChangeEventFunc(name: string, isChecked: boolean) {
    ...
    if (isChecked["checked"]) {
      ...
      const index = courseHoleSet.controls.findIndex((x) => x.value === name);
  ...
  existInList(name: string) {
    let courseHoles = this.formArray.get([0]).get("courseHoleSet") as FormArray;
  ...
  setupMarshals(selectedValue) {
    if (selectedValue != Constants.SM_ONLY_PLAYERS) {
      ...
      this.formArray
        ...
        .get("noofMarshals")
        .setValidators(Validators.required);
      this.formArray.get([0]).get("marshalStart").updateValueAndValidity();
      this.formArray.get([0]).get("noofMarshals").updateValueAndValidity();
    } else {
  ...
  async getSelectedPlayers() {
    ...
    for (
      ...
      for (let j in this.formArray.get([1]).get("category").value[i]
        ...
        this.formArray.get([1]).get("category").value[i].playingDate[j].Date =
          sDte;
  ...
  getNextFlighttee(k: number, index: number, category, flightData?) {
    ...
    if (startingHoleOption == "1") {
      ...
      let startingHole: number = parseFloat(
        (<HTMLInputElement>(
          document.getElementById("flight_" + k + "_" + (index - 1) + "_hole")
        )).value
      );
  ...
  getNextFlightTime(k: number, index: number, category, flightData?) {
    ...
    let dateNow: Date = new Date(
      Constants.DEFAULT_DATE +
        " " +
        (index == 0 ? FilteredFlight[0].flightStartTime : this.preFlightTime) +
        ""
    );
    ...
    if (arrangements == "0") {
      ...
      this.preFlightTime = ("0" + h).slice(-2) + ":" + ("0" + m).slice(-2);
    } else {
      makeInterval
        ? dateNow.setMinutes(
            dateNow.getMinutes() -
  ...
  getGrossNetTop(selectedCTName, type) {
    if (
      ...
      if (type == "GT") return +this.formArray.get([0]).value.professionalsGT;
  ...
  async createTournament() {
    ...
    if (!this.tournamentID) this.tournamentID = UniqueIdGenerator.generate();
    ...
    for (var index in this.formArray.get([0]).value.clubctgies) {
      ...
      if (
        ...
        if (this.formArray.get([0]).value.handicapCats) {
          TCdata = {
            lowerLimitStart: 1,
            lowerLimitEnd: this.formArray.get([0]).value.handicapCats
              ? this.formArray.get([0]).value.prizeCategoryA
              : "",
            upperLimitStart: this.formArray.get([0]).value.handicapCats
      ...
      if (
        ...
        if (this.formArray.get([0]).value.handicapRatio) {
          prizeInfo = {
            handicapratio: this.formArray.get([0]).value.handicapRatio,
          };
        }
      } else {
        if (
    ...
    let handicapAllocations={
      handicapAllocation: this.formArray.get([0]).value.handicapAllocations
    }
    ...
    let tournament = {
      ...
      courseId: this.formArray.get([0]).value.courseInfo[0]
        ? this.formArray.get([0]).value.courseInfo[0].courseName.course.id
        : "",
      ...
      courseHoleSets:
        courseHoleSetsData.length > 0 ? Number(courseHoleSetsData[0]) : 0,
      teamMatch: false,
      pairsMatch: false,
      interLeague: false,
      playingOnWhs: false,
    ...
    let result = <any>await this.facadeService.createTournamentMarshals(marshalsData);
...
let handicapAllocations={
    ...
    let result = <boolean>(
      await this.facadeService.editTournament(tournament, tournamentCats,marshalsData)
    );
    if (result) {
      ...
      .subscribe(
        (data) => {
          ...
          this.clubMembers = this.clubMembers.filter(
            (ar) => !this.tournamentMembers.find((rm) => rm.id === ar.id)
          );
          console.log(this.clubMembers);
  ...
  async createFlights() {
    ...
    if (this.currentTournament) this.tournamentID = this.currentTournament.id;
          ...
          for (var index3 in this.selectedMembers[index][index2]) {
            for (var index4 in this.selectedMembers[index][index2][index3]) {
              if (Number.isInteger(Number(index4))) {
                ...
                let FM: any = {
                  playerId:
                    this.selectedMembers[index][index2][index3][index4].id,
                  attendance: false,
            ...
            console.log(
              this.selectedMembers[index][index2][index3][index4].playerCategory
            );
            ...
            if (tournamentFlightMembers.length > 0) {
              ...
              let flight: any = {
                ...
                adminId: this.loggedInuser.id,
                courseHoleSets: this.currentTournament.courseHoleSets
                  ? this.currentTournament.courseHoleSets
                  : 0,
                flightNo: fcnter,
                flightRound: 1,
                tee_id: roundTeeId1.id,
                startingHole: this.selectedMembers[index][index2][index3].tee,
                tee: roundTeeId1.name,
                category: this.selectedMembers[index].title,
                date: General.parseToDate(
                  this.formArray.get([0]).value.startDateFormCtrl
                ),
                time: this.selectedMembers[index][index2][index3].time,
                ended: false,
