...
import { Player, IPlayerHandicapWhs, PlayerHanidcap, handicap_change_log } from '../../../../shared/models/player.model';
...
import { Constants, General, UniqueIdGenerator } from '../../../../shared/classes/general';
...
export class HandicapCalculationWhsComponent implements OnInit {
    ...
    private handicapsWhsRounds: Map<number, PlayerHandicapWhs[]> = new Map<number, PlayerHandicapWhs[]>();
    ...
    displayedColumns = ['id', 'name', 'oldhandicap', 'scores', 'adjustedScores', 'handicapDifferential'];

    @ViewChild(MatPaginator) paginator: MatPaginator;
    @ViewChild(MatSort) sort: MatSort;

    constructor(private router: Router, private route: ActivatedRoute, public snackBar: MatSnackBar, public dialog: MatDialog, public facadeService: FacadeService) { }
    ...
    async ngOnInit() {
      ...
      const tournamentInfo = await this.facadeService.getTournamentByID(tournamentId);
      //console.log(tournamentInfo.tournament[0]);

      this.tournamentScoresLoader = new TournamentRoundScoresLoader(tournamentInfo.tournament[0], this.facadeService);
    ...
    private updateData() : void {
        ...
        let noOfRounds: number = this.tournamentRounds = tournament.noOfRounds;
        ...
        for (let round: number = 1; round <= noOfRounds; round++) {
            ...
            let playerScores: Map<string, Score[]> = this.scoresQuery.getPlayerScores(round);
            ...
            let playerHandicapsWhs: Map<string, PlayerHandicapWhs> = this.scoresQuery.getPlayerHandicapsWhs(round);
            ...
            for (let playerEntry of Array.from(playerEntries)) {
                ...
                let playerCategoryLowerCased: string = player.playerCategory.toLowerCase();
                ...
                let scoresList: Score[] = playerScores.size > 0 && playerScores.get(playerId) ? playerScores.get(playerId) : [];
                ...
                if (holesPlayed < 14 && holesSize > 9) {
                    ...
                    hole9Set = this.adjustHolesAndScoresFor9HolesPlay(holes, scoresList);
                ...
                if (scoresList.length > 0) {
                    ...
                    for (let score of scoresList) {
                        ...
                        adjustedScores += holesPlayed <= 9 ? this.getWhsAdjustedGrossScoreFor9Holes(score.playerHandicap, score.grossScore, score["HoleIPQL"].par, score["HoleIPQL"].index) : this.getWhsAdjustedGrossScoreFor18Holes(score.playerHandicap, score.grossScore, score["HoleIPQL"].par, score["HoleIPQL"].index);
                    ...
                    if (missingHoles.size > 0) {
                        ...
                        for (let hole of Array.from(missingHolesValues)) {
                            ...
                            let strokes: number = this.getPlayerStrokes(handicap, hole.index);
                ...
                //console.log(adjustedScores);
                //console.log(playerHandicapsWhs);
                let playerHandicapWhs: PlayerHandicapWhs = playerHandicapsWhs.get(playerId);
                ...
                if (playerHandicapWhs == null) {
                    ...
                    playerHandicapWhs = new PlayerHandicapWhs(playerId, playerName, player.handicap, -1, null, roundDate, grossScores, adjustedScores, hole9Set == 1, hole9Set == 2, 0);
            ...
            let handicapWhsRound: PlayerHandicapWhs[] = this.handicapsWhsRounds.get(round);
        ...
        let playerHandicapsWhsMapTemp: Map<String, PlayerHandicapWhsAllRounds> = new Map<String, PlayerHandicapWhsAllRounds>();
        for (let round = 1; round <= noOfRounds; round++) {
            ...
            for (let playerHandicapWhs of handicapWhsRound) {
                ...
                let playerHandicapWhsAllRounds: PlayerHandicapWhsAllRounds = playerHandicapsWhsMapTemp.get(playerId);
                if (playerHandicapWhsAllRounds == null) {
                    playerHandicapWhsAllRounds = new PlayerHandicapWhsAllRounds(playerId, playerHandicapWhs.getName(), playerHandicapWhs.getOldHandicap(), noOfRounds);
                    playerHandicapsWhsMapTemp.set(playerId, playerHandicapWhsAllRounds);
                }
                else { 
                    if(round == 2) {
                        console.log(playerHandicapWhsAllRounds.getAdjustedScores(round - 1)); 
                        console.log(playerHandicapWhsAllRounds.getAdjustedScores(round)); 
                    }
                    
                }
                playerHandicapWhsAllRounds.setScore(playerHandicapWhs.getScores(), round);
                playerHandicapWhsAllRounds.setAdjustedScores(playerHandicapWhs.getAdjustedScores(), round);
                playerHandicapWhsAllRounds.setHandicapDifferential(playerHandicapWhs.getHandicapDifferential());
                playerHandicapWhsAllRounds.setHandicapIndex(playerHandicapWhs.getHandicapIndex());
            }
        }
        this.handicapsWhs = [];
    ...
    public removeExtraHoleSetsForPlayer(playerId: string, round: number, holes: Hole[]) : void {
    ...
    private adjustHolesAndScoresFor9HolesPlay(holes: Hole[], scoresList: Score[]) : number {
        let holesSize: number = holes.length;
    ...
    public getWhsAdjustedGrossScoreFor18Holes(playerHandicap: number, playerScore: number, holePar: number, holeIndex: number) : number {
    ...
    public getWhsAdjustedGrossScoreFor9Holes(playerHandicap: number, playerScore: number, holePar: number, holeIndex: number) : number {
        let handicap: number = playerHandicap;
        let score: number = playerScore;
        let par: number = holePar;
        if (score <= 0) {
    ...
    private updateWhsHandicaps() : void {
        ...
        let handicapsWhsRound: PlayerHandicapWhs[] = this.handicapsWhsRounds.get(this.whsCalculatingRound);
        ...
        for (let playerHandicapWhs of handicapsWhsRound) {
            ...
            let flight: Flight = this.scoresQuery.getPlayerFlight(playerHandicapWhs.getPlayerId(), this.whsCalculatingRound);
            if (!flight && this.whsCalculatingRound == 1) {
                flight = this.scoresQuery.getPlayerFlight(playerHandicapWhs.getPlayerId(), this.whsCalculatingRound - 1);
            }

            let rating: CourseRating = this.scoresQuery.getCourseRatingByFlight(flight);
            console.log("rating");
            console.log(rating);
            if (rating == null) {
                if (flight == null) {
                    rating = new CourseRating(this.tournament.courseId, Constants.DEFAULT_TEE, 0, courseRating, slopeRating, coursePar);
                } else {
                    rating = new CourseRating(this.tournament.courseId, flight.tee, flight.courseHoleSets, courseRating, slopeRating, coursePar);
                }
            }
            let handicapDifferential: number = (playerHandicapWhs.getAdjustedScores() - rating.getCourseRating()) * (113.0 / rating.getSlopeRating());
            handicapDifferential = General.truncateDecimals(handicapDifferential, 2);
            
            playerHandicapWhs.setHandicapDifferential(Number(handicapDifferential));
        ...
        //if(this.tournament && (this.tournament.clubId == "-MX5z42VVuYx0cV1CmOX" || this.tournament.clubId == "-MX62FWAPOgTOm0CCgA3" || this.tournament.clubId == "-LXJoCjyCi2CCRohUB5z" || this.tournament.clubId == "-L6W3NMaquXx0dJougVv" || this.tournament.clubId == "-LUFS3FAg4OEhIiK0vgY")) {
            this.savePlayerWhsHandicapsForRound(this.tournament.id, handicapsWhsRound, this.whsCalculatingRound);
        //}
        
        this.loadWhsHandicapHistory();
    }

    savePlayerWhsHandicapsForRound(tournamentId: string, handicapsWhs: PlayerHandicapWhs[], round: number) {
       ...
       let handicapWhsInputs: any[] = [];
        ...
        for (let handicap of handicapsWhs) {
            ...
            let handicapWhsInput = {
                ...
                front9: false,
                back9: false,
                handicapIndex: (handicap.getHandicapIndex())? handicap.getHandicapIndex() : 0,
                exceptionalScore: 0
            }
    ...
    private async loadWhsHandicapHistory() {
        ...
        this.playersWhsHandicapHistoryLoader = new PlayersWhsHandicapHistoryLoader(this.facadeService);
        await this.playersWhsHandicapHistoryLoader.resetParams();
        
        await this.playersWhsHandicapHistoryLoader.setParams(playerIds, playingDate);
    ...
    private loadedWhsHandicapHistory() : void {
        ...
        let playerHandicapsWhs: PlayerHandicapWhs[] = this.handicapsWhsRounds.get(this.whsCalculatingRound);
        //console.log(playerHandicapsWhs);
        ...
        for (let handicapWhs of playerHandicapsWhs) {
            ...
            let mapPlayerWhsHandicapHistory = this.playersWhsHandicapHistoryLoader.getPlayerWhsHandicapHistory(playerId);
            //console.log(mapPlayerWhsHandicapHistory);
            let playerWhsHandicapHistory: PlayerHandicapWhs[] = [];
            
            for(let hWhsRound of (mapPlayerWhsHandicapHistory)? Array.from(mapPlayerWhsHandicapHistory.values()) : []) {
                playerWhsHandicapHistory.push(hWhsRound);
            }
            
            if (playerWhsHandicapHistory == null || playerWhsHandicapHistory.length < 1) {
                continue;
            }
            ...
            let playerHandicapWhsList: PlayerHandicapWhs[] = playerWhsHandicapHistory.slice(0, 20);
            ...
            switch (handicapsAvailable) {
                ...
                case 20: handicapsToUse = 8; break;
                case 19: handicapsToUse = 7; break;
                case 18: case 17: handicapsToUse = 6; break;
                case 16: case 15: handicapsToUse = 5; break;
                case 14: case 13: case 12: handicapsToUse = 4; break;
                case 11: case 10: case 9: handicapsToUse = 3; break;
                case 8: case 7: case 6: handicapsToUse = 2; break;
                default: handicapsToUse = 1; break;
            }
            ...
            for (let i = 0; i < handicapsToUse; i++) {
                let playerHandicapWhs: PlayerHandicapWhs = playerHandicapWhsList[i]; //playerHandicapWhsList.get(i);
                sumOfLowestHandicaps += playerHandicapWhs.getHandicapDifferential();
            }
            let averageOfLowestHandicaps: number = sumOfLowestHandicaps / handicapsToUse;
            ...
            let truncatedToTenthDecimal: number = (averageOfLowestHandicaps * 10) / 10;
        ...
        for (let playerHandicap of this.handicapsWhs) {
            ...
            let handicapIndex: number = this.playerHandicapIndices.get(playerHandicap.getPlayerId());
            if (handicapIndex != null) {
                ...
                let handicapWhsInput = {
                    ...
                    handicapDifferential: playerHandicap.getHandicapDifferential(),
                    updatedAt: "2021-05-05",
                    playedAt: "2021-05-05",
                    score: playerHandicap.getScore(this.whsCalculatingRound),
                    adjustedScore: playerHandicap.getAdjustedScores(this.whsCalculatingRound),
                    front9: false,
                    back9: false,
        ...
        if (this.whsCalculatingRound < this.tournament.noOfRounds) {
            ...
            for (let entry of Array.from(entries)) {
                ...
                if (player != null) {
                    ...
                    if (handicapOld != handicap) {
                        changeLogs.push(new HandicapChangeLog(UniqueIdGenerator.generate(), player.id, handicapOld, handicap, true, changeLogRemarks, tournamentId, updaterId));
                    }
                }
            }
            ...
            //if(this.tournament && (this.tournament.clubId == "-MX5z42VVuYx0cV1CmOX" || this.tournament.clubId == "-MX62FWAPOgTOm0CCgA3" || this.tournament.clubId == "-LXJoCjyCi2CCRohUB5z" || this.tournament.clubId == "-L6W3NMaquXx0dJougVv" || this.tournament.clubId == "-LUFS3FAg4OEhIiK0vgY")) {
                this.facadeService.savePlayersHandicapWhsIndex(playersList, changeLogs);
            //}
            this.updatedWhHandicaps();
    ...
    private combine9HoleScorecards(playerWhsHandicapHistory: Array<PlayerHandicapWhs>, playerId: string) {
        //console.log(playerWhsHandicapHistory);
        for (let i: number = playerWhsHandicapHistory.length; i >= 0; i--) {
            let playerHandicapWhs: PlayerHandicapWhs = playerWhsHandicapHistory[i - 1];
            //console.log(playerHandicapWhs);
            let courseId: string = (playerHandicapWhs)? playerHandicapWhs.getCourseId() : "";
            console.log(courseId);
            if(!courseId) return;
            if (playerHandicapWhs.isFront9()) {
                let back9s: Array<PlayerHandicapWhs> = this.back9sOfPlayers.get(playerId + courseId);
                if (back9s == null || back9s.length == 0) {
                    let  front9s: Array<PlayerHandicapWhs> = this.front9sOfPlayers.get(playerId + courseId);
                    if (front9s == null) {
                        front9s = [];
                    ...
                    let getRemovedElement: any =  playerWhsHandicapHistory.splice((i - 1), 1); // need to debug
                ...
                if(back9) {
                    ...
                    playerHandicapWhs.addAdjustedScores(back9[0].getAdjustedScores());
                }
                
                let rating: CourseRating = playerHandicapWhs.getCourseRating();
                let handicapDifferential: number;
                if (rating != null) {
                    handicapDifferential = (playerHandicapWhs.getAdjustedScores() - rating.getCourseRating()) * (113.0 / rating.getSlopeRating());
                } else {
                    handicapDifferential = playerHandicapWhs.getAdjustedScores() - 72;
                }
                handicapDifferential = General.precisionRound(handicapDifferential, 2);
            ...
            if (playerHandicapWhs.isBack9()) {
                ...
                if(front9.length > 0) {
                    ...
                    playerHandicapWhs.addAdjustedScores(front9[0].getAdjustedScores());
                }
    ...
    private updateWhsScores() {
        ...
        let playerEntries = players.values();
        let playerScores: Map<string, Score[]> = this.scoresQuery.getPlayerScores(round);
        let handicapsWhsRound: PlayerHandicapWhs[] = this.handicapsWhsRounds.get(round);
        let handicapsWhsRoundMapTemp: Map<string, PlayerHandicapWhs> = new Map<string, PlayerHandicapWhs>();
        for(let playerHandicapWhs of handicapsWhsRound) {
            handicapsWhsRoundMapTemp.set(playerHandicapWhs.getPlayerId(), playerHandicapWhs);
        }
        ...
        for (let entry of Array.from(playerEntries)) {
            ...
            let handicapIndexCalculated: number = this.playerHandicapIndices.get(playerId);

            if (handicapIndexCalculated != null) {
                let newHandicap: number;
                let flight: Flight = this.scoresQuery.getPlayerFlight(playerId, this.whsCalculatingRound);
                
                let rating: CourseRating = this.scoresQuery.getCourseRatingByFlight(flight);
                if (rating != null) {
                    newHandicap = handicapIndexCalculated * (rating.getSlopeRating() / 113.0) + (rating.getCourseRating() - rating.getCoursePar());
                } else {
                    newHandicap = handicapIndexCalculated * (slopeRating / 113.0) + (courseRating - coursePar);
                }
            ...
            let playerHandicapWhs: PlayerHandicapWhs = handicapsWhsRoundMapTemp.get(playerId);
            if (playerHandicapWhs == null) {
                playerHandicapWhs = new PlayerHandicapWhs(playerId, playerName, player.handicap, -1, null, roundDate, grossScores, adjustedScores, hole9Set == 1, hole9Set == 2);
    ...
    private updatedWhHandicaps() : void {
        ...
        for (let round = 1; round <= noOfRounds; round++) {
            let playerHandicapsWhs: PlayerHandicapWhs[] = this.handicapsWhsRounds.get(round);
    ...
    Comparator(playerHandicapWhs1, playerHandicapWhs2) {
        let difference: number = playerHandicapWhs1.getHandicapDifferential() - playerHandicapWhs2.getHandicapDifferential();
        return difference < 0 ? -1 : difference > 0 ? 1 : 0;
    }
    ...
    public hasHoleSet1to9(flight: Flight) : boolean {
        return flight.courseHoleSets > 0 && (flight.courseHoleSets & this.Holes1to9) != 0;
    }

    public hasHoleSet10to18(flight: Flight) : boolean {
        return flight.courseHoleSets > 0 && (flight.courseHoleSets & this.Holes10to18) != 0;
    }

    public hasHoleSet19to27(flight: Flight) : boolean {
        return flight.courseHoleSets > 0 && (flight.courseHoleSets & this.Holes19to27) != 0;
    }

    public hasHoleSet28to36(flight: Flight) : boolean {
        return flight.courseHoleSets > 0 && (flight.courseHoleSets & this.Holes28to36) != 0;
    }
    ...
    setupCONGUHandicap() {
        ...
        .subscribe(async data => {
            let dataFullTournament = await this.facadeService.tournamentScoreLoader(tournamentId);
            ...
            if(this.roundPlayerScores.length > 0) this.calculateCONGUHandicap();
    ...
    parseTournamentScores(round: number) {
      ...
      calculateCONGUHandicap() {
        ...
        this.facadeService.savePlayerHandicaps(this.playerHandicapList, this.playersCongu, this.handicapChangelogList);
        this.isUpdated = true;
      }
