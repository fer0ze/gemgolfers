...
export class AddTeeTimesComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private facadeService: FacadeService, public snackBar: MatSnackBar) { }
...
console.log(this.scheduleForm.value);
      ...
      let isExist: TeeTime[] = await this.facadeService.isTeeTimeDateExist(clubId, General.parseToDate(this.scheduleForm.value.BookingDate));
      
      console.log(isExist["tee_time_booking"]);

      if(isExist["tee_time_booking"].length > 0) {
        this.snackBar.open("Selected date is already exist. Choose different.", "x", {
          duration: 5000,
        });
      ...
      let clubInfo: any = (this.loggedInuser.membership.length > 0)? this.loggedInuser.membership[0].club : [];
      console.log(clubInfo);
      let courseId: string = (clubInfo.length > 0 && clubInfo.courses.length > 0)? clubInfo.courses[0].id : "-KpFJ5_ODeRpEQCz9Drd";
      ...
      for(let slot of this.teeSlots) {
        let tee1 : any = {
          ...
          slotTime: slot,
          joinedMembers: 0,
          startingHole: (this.scheduleForm.value.startingHole == "both")? 1 : Number(this.scheduleForm.value.startingHole),
          flightId: null
        }
  ...
  generateTeeTimes() {
    try {
      this.teeSlots = [];
      let startLimit : Date = new Date(Constants.DEFAULT_DATE + " " + this.scheduleForm.value.startTime.substr(0,5));
      let endLimit : Date = new Date(Constants.DEFAULT_DATE + " " + this.scheduleForm.value.endTime.substr(0,5));
      console.log(startLimit);
      while(startLimit <= endLimit) {
        var h = startLimit.getHours();
        var m = startLimit.getMinutes();

        this.teeSlots.push(('0' + h).slice(-2) + ":" + ('0' + m).slice(-2));
