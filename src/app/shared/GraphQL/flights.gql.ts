...
export const SaveRoundFlightsMutation = gql`
    mutation SaveRoundFlightsMutation($flightsToSave: [flight_insert_input!]!) {
...
`;
export const insertFlightMembers = gql`
...
export const UpsertFlightAndMembersMutation = gql`
    mutation UpsertFlightAndMembersMutation(
        ...
        DeleteScoreQL: delete_score(where: $scoresDeleteExpression)
            @include(if: $deleteAndInsertScores) {
            AffectedRowsQL: affected_rows
        }
...
export const DeleteFlightsAndMembersMutation = gql`
    mutation DeleteFlightsAndMembersMutation($flightIdsToRemove: [String!]!) {
        FlightDeleteQL: delete_flight(
            where: { id: { _in: $flightIdsToRemove } }
        ) {
            AffectedRowsQL: affected_rows
        }
    }
...
`;
export const deletePlayerHandiCalQueryQL = gql`
    mutation DeletePlayerHandicapCal(
        $tournamnetId: String!
        $PlayersIds: [String!]!
    ) {
        DeleteQL: delete_player_handicap(
            where: {
                _and: [
                    {
                        tournamentId: { _eq: $tournamnetId }
                        playerId: { _in: $PlayersIds }
                    }
                ]
            }
        ) {
        ...
        DeleteQLs: delete_handicap_change_log(
            where: {
                _and: [
                    {
                        tournamentId: { _eq: $tournamnetId }
        ...
        DeleteQLS: delete_player_handicap_whs(
            where: {
                _and: [
                    {
                        tournamentId: { _eq: $tournamnetId }
