  ...
  mutation SavePlayersListMutation(
    ...
    PlayerEntryQLi: insert_player(
      ...
      on_conflict: {
        ...
        update_columns: [phone, email, firstName, lastName, membershipNumber]
      }
    ) {
...
export const UpdateMutation = gql`
  mutation updateMutation(
    ...
    insert_club_member(
      objects: [{ clubId: $clubID, playerId: $playerID, suspended: $suspended }]
      on_conflict: { constraint: club_member_pkey, update_columns: [suspended] }
    ) {
      returning {
        playerId
      }
...
export const AllPlayersByCategoryQL = gql`
  query playersByCategory {
    ...
    Professionals: player(where: { playerCategory: { _eq: "Professionals" } }) {
      id
    }
...
export const PlayerHandicapQuery = gql`
  query PlayerHandicapQuery($playerId: String!) {
    PlayerQL: player_by_pk(id: $playerId) {
      firstName
      id
      HandicapHistoryWhsQL: handicap_history_whs(order_by: { playedAt: desc }) {
        ...PlayerHandicapWhsQL
      }
    }
  }
