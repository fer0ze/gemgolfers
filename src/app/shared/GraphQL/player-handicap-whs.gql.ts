...
import { PlayerQL, PlayerHandicapQL, PlayerHandicapWhsQL } from '../fragments/player.fragment';
...
}${TournamentQL}${CourseQL}${CourseRatingQL}${HoleQL}${FlightsQL}${PlayerQL}${ScoreQL}${PlayerHandicapQL}${PlayerHandicapWhsQL}`;

export const PlayersHandicapWhsHistoryQueryQL = gql`
query PlayersHandicapWhsHistoryQuery($playerIds: [String!]!, $playingDate: timestamptz!) {
    PlayerQL: player(
        where: {id: {_in: $playerIds}}
    ) {
        id
        HandicapHistoryWhsQL: handicap_history_whs (
            where: {
                playedAt: {_lte: $playingDate}
            },
            order_by: [
                {playedAt: desc}
            ], limit: 40
...
}${PlayerHandicapWhsQL}${CourseQL}`;

export const SavePlayerWhsHandicapsMutationQL = gql`
mutation SavePlayerWhsHandicapsMutation($handicapsWhs: [player_handicap_whs_insert_input!]!) {
...
mutation UpdatePlayersHandicapWhsIndexMutation($playersWithHandicapWhsIndex: [player_insert_input!]!, $handicapChangeLog: [handicap_change_log_insert_input!]!) {
